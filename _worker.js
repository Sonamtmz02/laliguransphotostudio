...
async function fetchDocStamp(ref){
  try{
    ...
    return (j&&j.fields&&j.fields.updatedAt&&(j.fields.updatedAt.timestampValue||""))||"";
  }catch(e){return "";}
}
async function getStamp(){
  const [a,b,c,d]=await Promise.all([
    fetchStamp("products"),
    fetchStamp("categories"),
    fetchDocStamp("storeInfo/main"),
    fetchDocStamp("businessHours/weekly")
  ]);
  return a+"|"+b+"|"+c+"|"+d;
}
...
function faqAutoFor(cat){ if(!cat) return []; const n = cat.name; return [ [n + " को order कसरी गर्ने?", n + " order गर्नको लागि website मा यस category का products हेरेर WhatsApp वा call बाट order गर्नुहोस् — हामी तपाईंको order अनुसार नयाँ बनाइदिन्छौं।"], [n + " मा के-के customize गर्न मिल्छ?", n + " मा photo, size, रङ र design तपाईंको रोजाइ अनुसार customize गर्न मिल्छ। विस्तृत जानकारीको लागि WhatsApp मा सोध्नुहोस्।"], [n + " कति दिनमा तयार हुन्छ?", n + " made-to-order भएकाले समय work-load मा भर पर्छ। Order गर्दा WhatsApp बाट तयार हुने समय भनिदिन्छौं।"], [n + " को price कति छ?", n + " का products को price size र design अनुसार फरक हुन्छ। हालको price यस page का products मा देखिन्छ; थप जानकारीको लागि WhatsApp गर्नुहोस्।"] ]; }
function faqFor(p, cat){ const sl = cat ? slugify(cat.name) : ""; return FAQ_GENERAL.concat(FAQ_BY_CAT[sl] || faqAutoFor(cat)); }
