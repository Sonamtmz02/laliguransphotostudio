...
function toast(m) { let t = document.getElementById("lgToast"); if (!t) { t = document.createElement("div"); t.id = "lgToast"; t.style.cssText = "position:fixed;top:4.6rem;left:50%;transform:translateX(-50%);background:#221f1e;color:#fff;padding:.7rem 1.1rem;border-radius:999px;z-index:99;font-size:.8rem;font-weight:700;box-shadow:0 8px 24px rgba(0,0,0,.3);transition:opacity .3s"; document.body.appendChild(t); } const tb = document.getElementById("themeToggle"); if (tb) { const r = tb.getBoundingClientRect(); t.style.top = (r.bottom + 10) + "px"; t.style.left = "auto"; t.style.transform = "none"; t.style.right = Math.max(8, window.innerWidth - r.right) + "px"; } const _dk = resolveTheme(state.theme) === "dark"; t.style.background = _dk ? "#ffffff" : "#221f1e"; t.style.color = _dk ? "#221f1e" : "#fff"; t.textContent = m; t.style.opacity = "1"; clearTimeout(t._h); t._h = setTimeout(() => { t.style.opacity = "0"; }, 2200); }
...
const KT_FMT = new Intl.DateTimeFormat("en-US", { timeZone: "Asia/Kathmandu", year: "numeric", month: "2-digit", day: "2-digit", weekday: "long", hour: "2-digit", minute: "2-digit", hour12: false });
function ktParts() { return KT_FMT.formatToParts(new Date()); }
...
function init() {
  ...
  const _sbbCart = $("sbbCart"); if (_sbbCart) _sbbCart.addEventListener("click", () => { if (state.pmId) addToCart(state.pmId, state.pmSize || null); });

  $("productGrid").addEventListener("click", e => { const f = e.target.closest("[data-fav]"); if (f) { toggleFav(f.dataset.fav); return; } if (e.target.closest("[data-share]")) return; if (e.target.closest("[data-cart]")) return; if (e.target.closest("a")) return; const card = e.target.closest(".p-card"); if (card && card.dataset.id) { const p = state.products.find(x => x.id === card.dataset.id); if (p) { history.pushState({}, "", "/product/" + (p.slug || "")); route(); } } });
