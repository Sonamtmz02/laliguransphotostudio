...
function assignSlugsInPlace(list) { const used = {}; for (const p of [...list].sort((a,b)=>(a.createdAt?.toMillis?.()||0)-(b.createdAt?.toMillis?.()||0))) { let s = p.slug; if (!s) { let base = slugify(p.name), n = 2; s = base; while (used[s]) { s = base + "-" + n; n++; } } while (used[s]) { s = s + "-2"; } used[s] = true; p.slug = s; } }
...
function bizStatus() { const d = state.hours; if (!d) return { open: null, text: "" }; if (d.override && d.override.enabled) return { open: d.override.status === "open", text: d.override.status === "open" ? "Open" : "Closed" }; const now = ktNow(), days = d.days || {}, today = days[now.day]; if (today && today.open) { if (now.min >= toMin(today.opens) && now.min < toMin(today.closes)) return { open: true, text: "Open now · closes " + fmt12(today.closes) }; if (now.min < toMin(today.opens)) return { open: false, text: "Closed · opens today " + fmt12(today.opens) }; } const order = DAYS.map(x => x[0]), idx = order.indexOf(now.day); for (let i=1;i<=7;i++){ const k = order[(idx+i)%7]; if (days[k] && days[k].open) return { open: false, text: "Closed · opens " + DAYS.find(x=>x[0]===k)[1] + " " + fmt12(days[k].opens) }; } return { open: false, text: "Closed" }; }
...
function renderCollections() { const el = $("colGrid"); const cats = state.categories; if (!cats) { el.innerHTML = ""; return; } el.innerHTML = cats.map(c => { const img = imgUrl((state.products.find(p => p.categoryId === c.id && p.imageUrl) || {})).imageUrl || imgUrl(((state.products||[]).find(p => p.categoryId === c.id && p.imageUrl) || {}).imageUrl); return `<button class="col-card" data-col="${c.id}">${img ? `<img src="${img}" alt="${esc(c.name)}" loading="lazy" decoding="async">` : `<span class="col-motif">❀</span>`}<span class="col-name">${esc(c.name)}</span></button>`; }).join(""); }
...
async function loadProductsPage(reset) {
  ...
  try {
    let q = db.collection("products").orderBy("createdAt", "asc").limit(PAGE_SIZE);
...
/* ===== RECENTLY VIEWED (localStorage cache only) ===== */
...
/* ===== RELATED PRODUCTS ===== */
function renderRelated(p) { const el = $("ppRelated"); if (!el) return; const rel = state.products.filter(x => x.id !== p.id && x.categoryId === p.categoryId).slice(0, 4); el.parentElement.hidden = rel.length === 0; el.innerHTML = rel.map(productCard).join(""); }
...
async function findProductBySlug(slug) {
  ...
  if (p) return p;
  try { const snap = await db.collection("products").where("slug", "==", slug).where("isActive", "==", true).limit(1).get(); if (!snap.empty) { p = { id: snap.docs[0].id, ...snap.docs[0].data() }; state.products.push(p); } } catch (e) {}
  return p || null;
}
...
function showCategoryPage(slug) { const c = (state.categories||[]).find(x => catSlug(x) === slug); if (!c) { showLanding(); return; } $("landingMain").hidden = true; $("productView").hidden = true; $("categoryView").hidden = false; $("cvName").textContent = c.name; $("cvDesc").textContent = c.description || ""; const items = state.products.filter(p => p.categoryId === c.id); $("cvGrid").innerHTML = items.length ? items.map(productCard).join("") : `<p class="muted">Loading...</p>`; const url = location.origin + "/category/" + slug; setSeo({ title: `${c.name} | Laligurans Photo Studio`, desc: c.description || `Explore ${c.name} from Laligurans Photo Studio.`, image: "", url }); window.scrollTo(0,0); }
function route() { const path = location.pathname; const m = path.match(/^\/product\/([^\/]+)\/?$/); const c = path.match(/^\/category\/([^\/]+)\/?$/); if (m) showProductPage(decodeURIComponent(m[1])); else if (c) showCategoryPage(decodeURIComponent(c[1])); else showLanding(); }
...
function bindLive() {
  db.collection("categories").onSnapshot(s => { state.categories = s.docs.map(d => ({ id: d.id, ...d.data() })).filter(c => c.isActive).sort((a,b) => (a.displayOrder??0)-(b.displayOrder??0)); renderDrawer(); renderChips(); renderCollections(); renderRelated(state.pmId ? state.products.find(x=>x.id===state.pmId) || {} : {}); }, () => {});
  ...
  /* change detector: 1 read per change, resets product view */
  db.collection("products").limit(1).onSnapshot(s => { if (!s.metadata.fromCache && state._loadedOnce && !location.pathname.startsWith("/product/")) { state._loadedOnce = true; loadProductsPage(true); } state._loadedOnce = true; }, () => {});
}
...
function init() {
  ...
  document.addEventListener("click", e => {
    ...
    if (p.startsWith("/product/") || p.startsWith("/category/") || p === "/") { e.preventDefault(); history.pushState({}, "", p + (url.hash || "")); route(); if (url.hash) { const el = document.querySelector(url.hash); if (el) el.scrollIntoView({ behavior: "smooth" }); } }
  });
  ...
  $("catChips").addEventListener("click", e => { const b = e.target.closest(".chip"); if (!b) return; state.catFilter = b.dataset.cat; state.search = ""; $("searchInput").value = ""; renderChips(); renderDrawer(); if (state.searchMode) { state.searchMode = false; } loadProductsPage(true); });
