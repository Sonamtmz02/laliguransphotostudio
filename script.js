/* LALIGURANS USER PANEL v2 - premium dark, realtime */
...
/* theme */
function applyTheme(t) { document.body.classList.toggle("light", t === "light"); $("themeToggle").textContent = t === "light" ? "🌙" : "☀️"; localStorage.setItem("lgs_theme", t); }
...
function renderFavs() {
  const items = state.products.filter(p => state.favs.includes(p.id));
  $("favList").innerHTML = items.length ? items.map(p => `<button class="d-cat" data-favgo="${p.id}">♥ ${esc(p.name)} · ${fmtMoney(p.price)}</button>`).join("") : `<p class="muted">कुनै favorite छैन। Product को ♥ थिच्नुहोस्।</p>`;
}
...
function refreshBadge() { const s = bizStatus(); $("hoursStatus").innerHTML = s.open === null ? "" : (s.open ? `<span class="badge" style="background:rgba(31,175,90,.15);color:var(--wa)">● ${esc(s.text)}</span>` : `<span class="badge red">● ${esc(s.text)}</span>`); }
...
function renderDrawer() {
  $("drawerCats").innerHTML = `<button class="d-cat ${state.catFilter === "all" ? "active" : ""}" data-cat="all">All Products</button>` + state.categories.map(c => `<button class="d-cat ${state.catFilter === c.id ? "active" : ""}" data-cat="${c.id}">${esc(c.name)}</button>`).join("");
}
...
function renderProducts() {
  ...
  $("productGrid").innerHTML = items.length ? items.map(p => {
    ...
    const wa = productWa(p); const fav = state.favs.includes(p.id);
    return `<article class="p-card">
      ${img ? `<img src="${img}" alt="${esc(p.name)}" loading="lazy" decoding="async">` : `<div class="p-noimg">📷</div>`}
      <div class="p-body">
        <div class="p-top"><strong>${esc(p.name)}</strong><button class="p-fav ${fav ? "on" : ""}" data-fav="${p.id}" aria-label="Favorite">${fav ? "♥" : "♡"}</button></div>
...
function renderStore() {
  ...
  $("brandName").textContent = parts[0] ? name.split(" ")[0] || name : name;
  ...
  const wan = waNumber();
  $("aboutWa").hidden = !wan; $("aboutWa").querySelector("span").textContent = s.phone ? "+" + wan : "";
  $("dPhone").textContent = s.phone || ""; $("dWaPhone").textContent = s.phone ? "+" + wan : "";
  setLink("dCall", s.phone ? "tel:" + s.phone.replace(/[^+\d]/g,"") : "");
  setLink("dWa", generalWa());
  setLink("qCall", s.phone ? "tel:" + s.phone.replace(/[^+\d]/g,"") : "");
  setLink("qWa", generalWa());
...
function init() {
  ...
  $("productGrid").addEventListener("click", e => { const f = e.target.closest("[data-fav]"); if (f) toggleFav(f.dataset.fav); });
  ...
  document.addEventListener("keydown", e => { if (e.key === "Escape") { $("lightbox").hidden = true; closeDrawers(); } });
